...
export const GettingAllCompany = createAsyncThunk(
  "companys/GettingAllCompany",
  async () => {
    try {
...
export const GettingCompanyById = createAsyncThunk(
  "companys/GettingCompanyById",
  async (data) => {
    try {
  ...
  async (data) => {
    try {
      const response = await AxiosInstanceMultipart.put(
        `companyManage/compamy/${data.comId}`,
        data.formData
      );
...
export const GettingCompanyBySup = createAsyncThunk(
  "companys/GettingCompanyBySup",
  async (data) => {
    try {
      const response = await AxiosInstance.get(
        `companyManage/companybysup/${data}`
      );
        ...
        (state, action) => {
          ...
          if (action.type.includes("GettingAllCompany")) {
            ...
            if (action.payload.data)
              state.companys = [...state.companys, action.payload.data].sort(
                (a, b) => new Date(b.UpdateDate) - new Date(a.UpdateDate)
              );
            ...
            // ✅ เช็กก่อนว่าการอัปเดตสำเร็จไหม
            if (action.payload.status && action.payload.data) {
              state.companys = state.companys
                .map((companys) =>
                  companys.CompanyId === action.payload.data.CompanyId
                    ? action.payload.data
                    : companys
                )
                .sort(
            ...
            state.companys = state.companys.filter(
              (companys) => companys.CompanyId !== action.payload.data.CompanyId
            );
