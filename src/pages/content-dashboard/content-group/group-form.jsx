...
const GroupForm = ({ groupId }) => {
  ...
  const handleOpenModal = async () => {
    ...
    const supFilter = response?.payload?.data.filter(
      (sup) => sup.Active === true
    );
    ...
    if (groupId) {
      ...
      const supMap = response?.payload?.data?.ProductGroupSup?.map(
        ...
        await setFileList([
          {
            ...
            url: `${import.meta.env.VITE_REDIRECT_IMG}/images/${
              response?.payload?.data?.ImageName
            }`,
  ...
  const handleFormSubmit = async () => {
    try {
      ...
      if (supSelected.length === 0) {
        setValidSup("Please complete all the required information.");
        hasError = true;
      }
      if (!fileList || fileList.length === 0) {
        setValidImage("Please complete all the required information.");
        hasError = true;
      }
      ...
      confirm({
        ...
        content: (
          ...
          //     >
          //       {groupId
          //         ? "Do you want to update this product group?"
          //         : "Do you want to create a product group?"}
          //     </div>
          //     <div style={{ marginTop: 8, color: "#666" }}>
          //       {groupId
          //         ? "Confirm to proceed with updating the product group."
          //         : "Confirm to proceed with creating a product group."}
          //     </div>
          //   </div>
        ...
        onOk: async () => {
          try {
            ...
            const modal = success({
              ...
              content: (
                <div style={{ textAlign: "center", padding: 24 }}>
                  ...
                  >
                    {groupId ? "Updated successfully" : "Created successfully"}
                  </div>
                  <Button
                    type="default"
            ...
            // ถ้าต้องการ reset form ให้ทำที่ onOk ของ success หรือหลังเรียก success
          } catch (err) {
            Modal.warning({
              icon: null,
              title: null,
      ...
      >
        <div className="flex justify-start mb-2 gap-[20px] items-center">
          ...
          <div>
            ...
            <span className="block">
              The Product Group is currently in use and available.
            </span>
          </div>
        </div>
        <div className="my-3">
          <InputComponet
            ...
            placeholder="Add Product Group name"
            OnChange={setGroupNameEn}
            vildate={validNameEN}
            value={groupNameEn}
            maxLength={100}
            required
        ...
        <div className="my-3">
          <MultiSelect
            validate={validSup}
            label="Supplier"
            defaultValue={supSelected}
            placeholder="Select Supplier"
            options={suplierOptions}
            onChange={(e) => setSupSelectd(e)}
            required
          />
        </div>
        <div className="mt-4">
        ...
        <div className="w-full mt-1">
          ...
          >
            ...
            <p className="ant-upload-hint">JPG, PNG (Maximum size 5 mb)</p>
          </Dragger>
          <div className="mt-1 flex justify-end">
            {validImage && (
              <span className="text-red-500 text-sm">{validImage}</span>
            )}
          </div>
        </div>
        <div className="w-full flex justify-end mt-4">
          <div className="gap-x-3 flex w-[30%] 2xl:w-[30%] items-center">
            ...
            <ButtonFullComponent
              size="large"
              otherStyle="text-nowrap w-full text-white"
              lable={`${
                groupId ? "Update Product Group" : "Create New Product Group"
              }`}
