...
import {
  ToastifyError,
  ToastifySuccess,
} from "../../../../components/content-alert/toastify";
import {
  createProduct,
  getProductById,
  editProductById,
  deleteProductFile,
  deleteProductFolder,
} from "../../../../slicers/productsaleSlicer";
...
const ProductForm = () => {
  ...
  const currentSupplier = useSelector(
    (state) => state.productsale.singleProduct
  );
  ...
  const addComponent = () => {
    if (folderNameTH.trim() === "" || folderNameEN.trim() === "") {
      setFolderNameError("Please enter folder name");
      return;
    }
    setComponents([
      ...components,
  ...
  const renameComponent = (id, newNameEN, newNameTH) => {
    setComponents(
      components.map((comp) =>
        comp.id === id
          ? { ...comp, folderNameEN: newNameEN, folderNameTH: newNameTH }
          : comp
      )
    );
  };
  ...
  const CreateNewProduct = async () => {
    try {
      ...
      if (!productNameEN || !supplier || !Group) {
        ...
        await scrollToTop();
        productNameTH === ""
          ? setValidNameTH("Please complete all the required information.")
          : setValidNameTH("");
        productNameEN === ""
          ? setValidNameEN("Please complete all the required information.")
          : setValidNameEN("");
        productNo === ""
          ? setProNovalid("Please complete all the required information.")
          : setProNovalid("");
        // company === '' ? setvalidCom('Please complete all the required information.') : setvalidCom('');
        supplier === null
          ? setValidselectSup("Please complete all the required information.")
          : setValidselectSup("");
        Group === null
          ? setvalidselectGroup("Please complete all the required information.")
          : setvalidselectGroup("");
        model === null
          ? setvalidselectModel("Please complete all the required information.")
          : setvalidselectModel("");
        return;
      }
    ...
    const setVariable = async () => {
      try {
        ...
        const model = await dispatch(
          getModelBySup(Number(productArr?.SupplierId))
        );
        const sups = await dispatch(
          GettingSupByGroup(Number(productArr?.ProductGroupId))
        );
        ...
        await handleChangeCompany(
          `${productArr?.CompanyId},${productArr?.BuId}`
        );
          ...
          await setdefaultImageMain([
            {
              ...
              status: "done",
              url: `${import.meta.env.VITE_REDIRECT_IMG}/images/${
                productArr?.ProductImageMain
              }`,
              thumbUrl: `${import.meta.env.VITE_REDIRECT_IMG}/images/${
                productArr?.ProductImageMain
              }`,
            },
          ]);
          ...
          await setdefaultMedia([
            {
              ...
              status: "done",
              url: `${import.meta.env.VITE_REDIRECT_IMG}/videos/${
                productArr?.ProductUpVideo
              }`,
              thumbUrl: `${import.meta.env.VITE_REDIRECT_IMG}/videos/${
                productArr?.ProductUpVideo
              }`,
            },
          ]);
        ...
        await setdefaultImageChildren(
          productArr?.ProductImageChildren.map((items) => ({
            uid: items.productImageImage,
            name: items.productImageImage,
            status: "done",
            url: `${import.meta.env.VITE_REDIRECT_IMG}/images/${
              items.productImageImage
            }`,
            thumbUrl: `${import.meta.env.VITE_REDIRECT_IMG}/images/${
              items.productImageImage
            }`,
        ...
        await setdefaultPresentFile(
          productArr?.PresentFile.map((items) => ({
            uid: items.FileName,
            name: items.FileOriginalName,
            status: "done",
            url: `${import.meta.env.VITE_REDIRECT_IMG}/files/${items.FileName}`,
            thumbUrl: `${import.meta.env.VITE_REDIRECT_IMG}/files/${
              items.FileName
            }`,
          }))
        );
          ...
          ? await setComponents(
              folderArr.map((item) => ({
                ...
                file: item.ProductFiles.map((file) => ({
                  ...
                  status: "done",
                  url: `${import.meta.env.VITE_REDIRECT_IMG}/files/${
                    file.ProductFile
                  }`,
                  thumbUrl: `${import.meta.env.VITE_REDIRECT_IMG}/files/${
                    file.ProductFile
  ...
  const handleChangeModal = async (e) => {
    ...
    const supFilter = await sups.payload.data.filter(
      (sup) => sup.Supplier.Active === true
    );
    const supData = supFilter
      .map((sup) => ({
  ...
  return (
    <div className="mb-10 2xl:mx-[20px] md:mx-[30px]">
      <div className="mt-10 mb-3">
        <h1 className="font-primaryMedium text-[18px]">
          {id === undefined ? "Create New Product" : "Update a Product Detail"}
        </h1>
          ...
          <div className="w-full p-[20px]">
            <div className="flex justify-start mb-2 gap-[20px] items-center">
              ...
              <div>
                ...
                <span className="block">
                  The active status indicates that the product is currently
                  available
                </span>
              </div>
            </div>
            <div className="w-full grid grid-cols-2 gap-3">
              ...
              <SelectOption
                ...
                placeholder="Select Company & Bussiness unit"
                validate={validcom}
                label="Company & Bussiness unit"
            ...
            <div className="w-full mt-3">
              <div>
                <span className="text-[16px] font-primaryMedium">
                  Main Photo
                </span>
                <span className="block">
                  Add image for showing first product image
                </span>
              </div>
              {isLoading ? (
                <div>Loading</div>
                ...
                >
                  ...
                  <p className="ant-upload-hint">JPG, PNG (Maximum size 5mb)</p>
                </Dragger>
              )}
            </div>
            <div className="w-full mt-3">
              <div>
                ...
                <span className="block">
                  Add images for showing images about your product.
                </span>
              </div>
              {isLoading ? (
                <div>Laoding</div>
            ...
            <div className="mt-4">
              <InputComponet
                color="red"
                placeholder="Enter the description header"
                label="Description Header"
                maxLength={100}
                OnChange={setdescriptionHeaderEN}
                value={descriptionHeaderEN}
              />
              <ReactQuill
      ...
      <div className="mt-5 mb-3">
        ...
        <span>
          Allows users to upload presentation(PDF) files to the system for
          sharing or viewing
        </span>
      </div>
      <NormalCard>
          ...
          <div className="p-[20px] w-full">
            ...
            {isLoading ? (
              ...
              >
                ...
                <p className="ant-upload-hint">PDF (Maximum size 50mb)</p>
              </Dragger>
            )}
          </div>
        )}
      ...
      <div className="my-5 mb-3">
        ...
        <span>
          Add video, Media, or other meadia files to showcase or support your
          product
        </span>
      </div>
      <NormalCard>
        {isLoading ? (
          ...
          <div className="p-[20px] w-full">
            <div className="flex justify-between gap-x-2">
              <button
                className={`${
                  FormDevice ? "border-red-500 text-red-500" : ""
                } w-full py-1 px-2 border rounded-md duration-100 ease-in-out`}
                onClick={() => setFormDevice(true)}
              >
                <span>From Device</span>
              </button>
              <button
                className={`${
                  FormDevice === false ? "border-red-500 text-red-500" : ""
                } w-full py-1 px-2 border rounded-md duration-100 ease-in-out`}
                onClick={() => setFormDevice(false)}
              >
                <span>From URL</span>
              </button>
            </div>
            {FormDevice ? (
              ...
              <div className="mt-5">
                ...
                <div className="mt-3">
                  <InputComponet
                    color="red"
                    placeholder="Enter Video Link (youtube)"
                    label="video URL"
                    OnChange={setMeadiaLink}
                    maxLength={250}
                    value={MeadiaLink}
                  />
                </div>
                <div className="mt-3">
                  <TextArea
                    placeholder="Enter Video Descrition"
                    label="Description"
                    value={mediaDescription}
                    maxLength={5000}
                    OnChange={setMediaDescription}
      ...
      <div className="mt-5 mb-3">
        ...
        <span>
          Add PDF files format only to provide addition information about your
          project
        </span>
      </div>
      <NormalCard>
        {isLoading ? (
          ...
          <div className="w-full mt-2 p-[20px]">
            <div
              className={`w-full flex gap-x-3 border p-4 rounded-md ${
                folderNameError ? "items-center" : "items-end"
              }`}
            >
              <div className="w-full">
                ...
                >
                  <label className="block text-sm text-gray-700 mb-[2px]">
                    Folder Name
                  </label>
                  <Input
                    size="large"
