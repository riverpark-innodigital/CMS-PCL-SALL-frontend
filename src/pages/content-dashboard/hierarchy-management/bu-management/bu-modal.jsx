...
import {
  GettingCurrentBU,
  UpdatingBU,
} from "../../../../slicers/businessuintSlicer";
...
const BUModal = ({ id }) => {
  ...
  const handlerSubmit = async () => {
    try {
      if (!buname) {
        buname === ""
          ? setValidBuName("Please complete all the required information.")
          : setValidBuName("");
        return;
      }
  ...
  const SaveData = async () => {
    try {
      ...
      const response =
        id === undefined
          ? await dispatch(CreateNewBU(data))
          : await dispatch(UpdatingBU(updateData));
      ...
      setErrMSG(
        typeof error === "string"
          ? error
          : "เกิดข้อผิดพลาดขณะบันทึกข้อมูล โปรดลองอีกครั้ง"
      );
    ...
    <>
          ...
          <div className="flex justify-start mb-2 gap-[20px] items-center">
            ...
            <div>
              ...
              <span className="block">
                The Business Unit is available for use.
              </span>
            </div>
          </div>
      ...
      <SuccessDialog
        title={
          successMSG !== ""
            ? successMSG
            : BUSINESS_UNIT_SHECEMA.SUCCESS_MODAL_TITLE
        }
