...
const GroupPermissionContent = () => {
  ...
  const allGroupPermission = useSelector(
    (state) => state.permission.groupPermissions
  );
  ...
  useEffect(() => {
    ...
    if (allGroupPermission.length > 0) {
      setTableData(
        allGroupPermission.map((items, index) => ({
          key: index + 1,
          id: items?.id,
          ...
          CreateDateText: items?.createdDate
            ...
            : "-",
          UpdateDateText: items?.updatedDate
            ? dateFormat(items?.updatedDate, "mediumDate")
            : "-",
