...
const InputComponet = ({
  ...
  const handleChangeValue = (e) => {
    ...
    if (format === true) {
      if (
        regexThai.test(inputValue) &&
        inputValue.length <= (maxLength === undefined ? 255 : maxLength)
      ) {
        OnChange(inputValue);
        // OnChange(e);
      ...
      >
        ...
        {type !== "password" && (
          <Input
            ...
            suffix={error && <MdErrorOutline className="pointer-events-none" />}
        ...
        <div className="mx-1 flex justify-between">
          ...
          {vildate && !error && !typing && (
            <p className="text-red-500 text-[12px]">{vildate}</p>
          )}
        </div>
      </ConfigProvider>
    </div>
